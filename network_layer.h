...
typedef void (*set_payload_fn)(void *packet, const uint8_t *payload, uint32_t length);
typedef uint32_t (*serialize_fn)(const void *packet, uint8_t *buffer, uint32_t buffer_size, ...);
...
typedef struct {
    ...
    int (*send_udp_packet)(struct network_layer_t *net, 
                           uint16_t src_port, uint16_t dest_port, 
                           uint32_t dest_addr, const uint8_t *dest_mac, 
