...
static void pde_prt( uint32_t level, uint32_t i, uint32_t entry, bool_t all ) {
	...
	// indent
	for( int n = 0; n <= level; ++n ) cio_puts( "  " );
	...
	if( IS_LARGE(entry) ) {   // PS is 1
		if( (entry & PDE_PAT) != 0 ) cio_puts( " PAT" );
		if( (entry & PDE_G) != 0 ) cio_puts( " G" );
		cio_puts( " PS" );
		if( (entry & PDE_D) != 0 ) cio_puts( " D" );
	}
	if( (entry & PDE_A) != 0 )   cio_puts( " A" );
	if( (entry & PDE_PCD) != 0 ) cio_puts( " CD" );
	if( (entry & PDE_PWT) != 0 ) cio_puts( " WT" );
	if( (entry & PDE_US) != 0 )  cio_puts( " U" );
	if( (entry & PDE_RW) != 0 )  cio_puts( " W" );

	// frame address
	cio_printf( " P --> %s %08x\n",
			IS_LARGE(entry) ? "Pg" : "PT", PDE_ADDR(entry) );
}
...
static void pte_prt( uint32_t level, uint32_t i, uint32_t entry, bool_t all ) {
	...
	// perms
	if( (entry & PTE_G) != 0 )   cio_puts( " G" );
	if( (entry & PTE_PAT) != 0 ) cio_puts( " PAT" );
	if( (entry & PTE_D) != 0 )   cio_puts( " D" );
	if( (entry & PTE_A) != 0 )   cio_puts( " A" );
	if( (entry & PTE_PCD) != 0 ) cio_puts( " CD" );
	if( (entry & PTE_PWT) != 0 ) cio_puts( " WT" );
	if( (entry & PTE_US) != 0 )  cio_puts( " U" );
	if( (entry & PTE_RW) != 0 )  cio_puts( " W" );
			...
			if( dir ) {
				...
				if( mode > Simple ) {
					...
					if( !IS_LARGE(entry) && mode > OneLevel ) {
						ptdump( level+1, (void *)P2V(PTE_ADDR(entry)),
								false, mode );
					}
				}
			} else {
...
void vm_init( void ) {
	...
	for( uint32_t addr = 0; addr < NUM_4MB; addr += SZ_PAGE ) {
		int stat = vm_map( kpdir, (void *) addr, addr,
				SZ_PAGE, PTE_US|PTE_RW );
		if( stat != SUCCESS ) {
...
pde_t *vm_mkkvm( void )
{
	...
	for( int i = 0; i < n_kmap; ++i, ++k ) {
		int stat = vm_map( pdir, ((void *)k->va_start),
				k->pa_start, k->pa_end - k->pa_start, k->perm );
		if( stat != SUCCESS ) {
			vm_free( pdir );
			return 0;
		}
	...
	for( int i = 0; i < (N_PDE >> 1); ++i ) {
		...
		if( IS_PRESENT(entry) ) {
			...
			if( !IS_LARGE(entry) ) {
				...
				pte_t *newpt = (pte_t *) vm_pagedup(
						(void *)P2V(PTE_ADDR(entry)) );
				if( newpt == NULL ) {
					return E_NO_MEMORY;
				}
