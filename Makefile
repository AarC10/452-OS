...
DBG_OPTIONS = -DTRACE_CX -DT_INIT -DRPT_INT_UNEXP -DT_KM -DT_KMF

USER_OPTIONS = $(GEN_OPTIONS) $(DBG_OPTIONS) $(EXTRAS)
...
#
CC = gcc -pipe
CFLAGS = -m32 -fno-pie -std=c99 -fno-stack-protector -fno-builtin -Wall -Wstrict-prototypes -MD -O2 $(CPPFLAGS)
...
all:	lib bootstrap userland kernel util user.img disk.img
...
kernel.nl: obj/kernel/kernel
	nm -Bng obj/kernel/kernel.o | pr -w80 -3 > kernel.nl

kernel.nll: obj/kernel/kernel
	nm -Bn obj/kernel/kernel.o | pr -w80 -3 > kernel.nll

#
# Generate a disassembly
#

kernel.dis: obj/kernel/kernel
	objdump -d obj/kernel/kernel > kernel.dis
